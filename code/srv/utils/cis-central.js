...
if (cds.env.profiles.find( p =>  p.includes("hybrid") || p.includes("production"))) {
    services = xsenv.getServices({ sm: { label: 'service-manager',  plan : 'subaccount-admin' } });
}
...
class CloudManagementCentral{
    ...
    async createServiceInstance(serviceName, serviceOffering, servicePlan, parameters) {
        try {
            let body = {
                ...
                labels: { createdBy: [`${process.env["HELM_RELEASE"]}-${process.env["KYMA_NAMESPACE"]}-automator`] }
            };
            let token = await this.getToken();
            let optionsInstance = {
                method: 'POST',
                url: services.sm.sm_url + `/v1/service_instances`,
                headers: {
    ...
    async createServiceBinding() {
        try {
            ...
            let options = {
                method: 'POST',
                url: services.sm.sm_url + `/v1/service_bindings`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                },
                data: JSON.stringify({ name: `${process.env["HELM_RELEASE"]}-${process.env["KYMA_NAMESPACE"]}-automator`, service_instance_id: this.instanceDetails.id })
            };
    ...
    async deleteServiceInstance() {
        try {
            ...
            let optionsInstance = {
                method: 'DELETE',
                url: services.sm.sm_url + `/v1/service_instances/${this.instanceDetails.id}`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                }
    ...
    async deleteServiceBinding() {
        try {
            ...
            let optionsInstance = {
                method: 'DELETE',
                url: services.sm.sm_url + `/v1/service_bindings/${this.bindingDetails.id}`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                }
    ...
    async createServiceManager(tenant) {
        try {
            ...
            let authOptions = {
                ...
                data: JSON.stringify({
                    labels: {
                        createdBy: [`${process.env["HELM_RELEASE"]}-${process.env["KYMA_NAMESPACE"]}-automator`]
                    }
                })
            };
    ...
    async getToken() {
        try {
            if (!this.tokenStore.token) {
                let tokenEndpoint = services.sm.url + '/oauth/token';
                this.tokenStore.token = await this.uaa.getTokenWithClientCreds(tokenEndpoint, services.sm.clientid, services.sm.clientsecret);
            }

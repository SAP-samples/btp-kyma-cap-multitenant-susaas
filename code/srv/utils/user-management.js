...
class UserManagement {
    ...
    checkIASBinding() {
        try {
            xsenv.getServices({ ias: { label: 'identity' } });
            return true;
        } catch (error) {
            return false
        }
    ...
    async createUser(userInfo) {
        try {
            ...
            if (this.ias) {
                createdUserInfo.iasLocation = await this.createIASUser(userInfo);
            }
    ...
    async resetIASPassword(email) {
        try {
            let options = {
                method: 'POST',
                url: `${ias.url}/service/users/forgotPassword`,
                headers: {
            ...
            await axios(options);
            console.log("SAP IAS password has been reset of user with email ", email)
        } catch (error) {
    ...
    async deleteUser(user) {
        try {
            ...
            if (this.ias) {
                await this.deleteIASUser(user.iasLocation);
            }
        } catch (error) {
            throw error;
        }

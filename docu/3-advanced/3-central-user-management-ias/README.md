...
- [Central user management using SAP Identity Authentication Service](#central-user-management-using-sap-identity-authentication-service)
  ...
  - [4. Update Helm Charts and run Upgrade](#4-update-helm-charts-and-run-upgrade)
...
2.6. Once the IAS tenant is provisioned, it will also appear in the dropdown list of your **Trust Configuration** and you can finish the trust set up in your Consumer and Provider Subaccounts.
...
## 4. Update Helm Charts and run Upgrade 

As already described in the [Advanced Version Repository details](../1-understand-repo-details/README.md) chapter, a few changes to your Umbrella Helm Chart *values.yaml* file are required to enable the usage of SAP Identity Authentication Service. Enabling the following configuration will create a new **Cloud Identity** service instance of type **application** upon *helm install* or *helm upgrade*. 
...
4.3. Okay, once you're done with these to changes, please run a *helm upgrade* command, to update your workloads and to create the new **Cloud Identity** Service Instance and respective Service Bindings. 
...
4.4. This is it! All the magic is now happening under the hood. Feel free to check out the user-management utilities ([click here](../../../code/srv/utils/user-management.js)) to see which additional code snippets are being executed in case of an existing Service Binding to a Cloud Identity Service Instance. Just search for "**this.ias**" and you will see how SAP IAS is integrated into the user management/creation/deletion flow. 
...
- Helm Umbrella Chart updated and Helm Release successfully upgraded ([click here](#4-update-helm-charts-and-run-upgrade))
- Cloud Identity Service Instance and Service binding available in Kyma ([click here](#4-update-helm-charts-and-run-upgrade))
